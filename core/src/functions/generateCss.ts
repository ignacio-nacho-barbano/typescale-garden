import type { CssFont } from "../models/fonts.js";
import type { TypeVariant } from "../models/type-variants.js";

/**
 * The stylesheet shown in the Export modal.
 *
 * One deliberate difference from the original: it sorted `weights` IN PLACE and
 * then read `weights[0]` further down, so `body { font-weight }` is the LOWEST
 * weight rather than the first step. That output is preserved exactly (via
 * `sorted`), but the caller's array is no longer mutated — the original was
 * sorting the weightSteps store's own array out from under it.
 */
export const generateCss = (
	...
	return `@import url('https://fonts.googleapis.com/css2?family=${font.family.replaceAll(
		" ",
		"+"
	)}:wght@${Array.from(new Set(sorted)).join(";")}&display=swap');
